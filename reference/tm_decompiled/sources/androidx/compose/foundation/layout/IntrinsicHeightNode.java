...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo825calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int maxIntrinsicHeight;
        if (this.height == IntrinsicSize.Min) {
            maxIntrinsicHeight = measurable.minIntrinsicHeight(Constraints.m7342getMaxWidthimpl(j));
        } else {
            maxIntrinsicHeight = measurable.maxIntrinsicHeight(Constraints.m7342getMaxWidthimpl(j));
        }
        if (maxIntrinsicHeight < 0) {
            maxIntrinsicHeight = 0;
        }
        return Constraints.INSTANCE.m7353fixedHeightOenEA2s(maxIntrinsicHeight);
    }
