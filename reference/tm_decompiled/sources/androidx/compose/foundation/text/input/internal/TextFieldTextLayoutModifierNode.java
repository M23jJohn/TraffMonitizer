...
public final class TextFieldTextLayoutModifierNode extends DelegatingNode implements LayoutModifierNode, GlobalPositionAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public MeasureResult mo108measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        TextLayoutResult m1621layoutWithNewMeasureInputshBUhpc = this.textLayoutState.m1621layoutWithNewMeasureInputshBUhpc(measureScope, measureScope.getLayoutDirection(), (FontFamily.Resolver) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, CompositionLocalsKt.getLocalFontFamilyResolver()), j);
        final Placeable mo6055measureBRTryo0 = measurable.mo6055measureBRTryo0(Constraints.INSTANCE.m7351fitPrioritizingWidthZbe2FdA((int) (m1621layoutWithNewMeasureInputshBUhpc.getSize() >> 32), (int) (m1621layoutWithNewMeasureInputshBUhpc.getSize() >> 32), (int) (m1621layoutWithNewMeasureInputshBUhpc.getSize() & 4294967295L), (int) (m1621layoutWithNewMeasureInputshBUhpc.getSize() & 4294967295L)));
        TextLayoutState textLayoutState = this.textLayoutState;
        if (this.singleLine) {
            m7399constructorimpl = measureScope.mo456toDpu2uoSUM(TextDelegateKt.ceilToIntPx(m1621layoutWithNewMeasureInputshBUhpc.getLineBottom(0)));
        } else {
            m7399constructorimpl = Dp.m7399constructorimpl(0);
        }
        textLayoutState.m1622setMinHeightForSingleLineField0680j_4(m7399constructorimpl);
        LinkedHashMap linkedHashMap = this.baselineCache;
        if (linkedHashMap == null) {
            linkedHashMap = new LinkedHashMap(2);
        }
        linkedHashMap.put(AlignmentLineKt.getFirstBaseline(), Integer.valueOf(Math.round(m1621layoutWithNewMeasureInputshBUhpc.getFirstBaseline())));
        linkedHashMap.put(AlignmentLineKt.getLastBaseline(), Integer.valueOf(Math.round(m1621layoutWithNewMeasureInputshBUhpc.getLastBaseline())));
        this.baselineCache = linkedHashMap;
        int size = (int) (m1621layoutWithNewMeasureInputshBUhpc.getSize() >> 32);
        int size2 = (int) (m1621layoutWithNewMeasureInputshBUhpc.getSize() & 4294967295L);
