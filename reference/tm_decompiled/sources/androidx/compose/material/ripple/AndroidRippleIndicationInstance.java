...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int i;
        this.rippleSize = contentDrawScope.mo5121getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m1893getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo5121getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo453roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m4625unboximpl = this.color.getValue().m4625unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1899drawStateLayerH2RKhps(contentDrawScope2, this.radius, m4625unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1897setRipplePropertiesbiQXAtU(contentDrawScope.mo5121getSizeNHjbRc(), this.rippleRadius, m4625unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1896addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m4625unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
