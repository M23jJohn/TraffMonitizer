...
final class RippleNodeFactory implements IndicationNodeFactory {
    ...
    private RippleNodeFactory(boolean z, float f, ColorProducer colorProducer) {
        this(z, f, colorProducer, Color.INSTANCE.m4651getUnspecified0d7_KjU());
    }
    ...
    public boolean equals(Object other) {
        ...
        if (this.bounded == rippleNodeFactory.bounded && Dp.m7404equalsimpl0(this.radius, rippleNodeFactory.radius) && Intrinsics.areEqual(this.colorProducer, rippleNodeFactory.colorProducer)) {
            return Color.m4616equalsimpl0(this.color, rippleNodeFactory.color);
        }
        return false;
    }
    ...
    @Override // androidx.compose.foundation.IndicationNodeFactory
    public int hashCode() {
        int m = ((UByte$$ExternalSyntheticBackport0.m(this.bounded) * 31) + Dp.m7405hashCodeimpl(this.radius)) * 31;
        ColorProducer colorProducer = this.colorProducer;
        return ((m + (colorProducer != null ? colorProducer.hashCode() : 0)) * 31) + Color.m4622hashCodeimpl(this.color);
    }
}
