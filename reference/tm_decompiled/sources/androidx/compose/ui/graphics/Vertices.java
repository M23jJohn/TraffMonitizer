...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m4669toArgb8_81llA(colors.get(i).m4625unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            long m4353unboximpl = points.get(i / 2).m4353unboximpl();
            if (i % 2 == 0) {
                intBitsToFloat = Float.intBitsToFloat((int) (m4353unboximpl >> 32));
            } else {
                intBitsToFloat = Float.intBitsToFloat((int) (m4353unboximpl & 4294967295L));
            }
